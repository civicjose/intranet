...
import { MdClose, MdPerson, MdMail, MdPhone } from 'react-icons/md';
import { FaSpinner } from 'react-icons/fa';

// Componente auxiliar para los inputs (sin cambios)
...
const EditUserModal = ({ user, roles, departments: allDepartments, onSaveSuccess, onClose }) => {
  // --- CORRECCIÓN AQUÍ ---
  // Inicializamos el estado con la estructura completa y valores por defecto.
  // 'departments' siempre será un array.
  ...
  // Este useEffect ahora rellena el estado que ya tiene la forma correcta.
  useEffect(() => {
    if (user) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiClient.put(`/users/${user.id}`, dataToSave);
      onSaveSuccess();
    } catch (err) {
      setError(err.response?.data?.message || 'Error al guardar. Por favor, revisa los datos.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor="companyPhone" className="block text-sm font-medium text-gray-700 mb-1">Teléfono</label>
                <FormInput icon={<MdPhone className="text-gray-400"/>} type="tel" name="companyPhone" id="companyPhone" value={formData.companyPhone} onChange={handleChange} />
              </div>
            </div>

            <div>
              <label htmlFor="role_id" className="block text-sm font-medium text-gray-700 mb-1">Rol</label>
