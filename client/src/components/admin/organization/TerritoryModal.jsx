// client/src/components/admin/organization/TerritoryModal.jsx
import React, { useState, useEffect } from 'react';
import { MdClose } from 'react-icons/md';
import { FaSpinner } from 'react-icons/fa';
import ErrorMessage from '../../ErrorMessage';

const TerritoryModal = ({ territory, onSave, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await onSave({ id: territory?.id, name });
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <header className="flex justify-between items-center p-5 border-b bg-gray-50 rounded-t-lg">
          <h2 className="text-xl font-bold text-gray-800">
            {isEditing ? 'Editar Territorio' : 'Añadir Nuevo Territorio'}
          </h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-800"><MdClose size={24} /></button>
        </header>
        <form onSubmit={handleSubmit}>
          <div className="p-6">
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Nombre del Territorio</label>
            <input
              type="text"
              name="name"
              id="name"
