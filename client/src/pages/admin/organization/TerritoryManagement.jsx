...
const TerritoryManagement = () => {
  ...
  const handleSave = async (data) => {
    try {
      ...
      if (data.id) {
        await apiClient.put(`/territories/${data.id}`, payload);
      } else {
        await apiClient.post("/territories", payload);
      }
      handleCloseModal();
      fetchData();
    } catch (err) { throw err; }
  ...
  const handleDragEnd = async (event) => {
      ...
      if (active.id !== over.id) {
          const oldIndex = territories.findIndex((t) => t.id === active.id);
          const newIndex = territories.findIndex((t) => t.id === over.id);
          const newOrder = arrayMove(territories, oldIndex, newIndex);
          setTerritories(newOrder);
          try {
              const orderedIds = newOrder.map(t => t.id);
              await apiClient.put('/territories/reorder', { orderedIds });
          } catch (err) {
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <SortableContext items={territories.map(t => t.id)} strategy={verticalListSortingStrategy}>
