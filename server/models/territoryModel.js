...
class Territory {
  static async getAll() {
    const [rows] = await db.query('SELECT * FROM territories ORDER BY order_index ASC, name ASC');
    return rows;
  }
  ...
  static async create({ name, order_index }) {
    const [result] = await db.query('INSERT INTO territories (name, order_index) VALUES (?, ?)', [name, order_index || 100]);
    return { id: result.insertId, name, order_index };
  }

  static async update(id, { name, order_index }) {
    await db.query('UPDATE territories SET name = ?, order_index = ? WHERE id = ?', [name, order_index || 100, id]);
    ...
    try {
      ...
      await Promise.all(
        orderedIds.map((id, index) => 
          connection.query('UPDATE territories SET order_index = ? WHERE id = ?', [index + 1, id])
        )
      );
